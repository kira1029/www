...
class ControllerAccountAddress extends Controller {
	...
	protected function getList() {
		...
		foreach ($results as $result) {
			if ($result['address_format']) {
				...
				$format = '姓名' . ":" . '{firstname}' . "\n" . '公司' . ":" . '{company}' . "\n" . '地址' . ":" . '{country}' . '{zone}' . '{city} ' . '{address_1}' . "\n" . '行业' . ":" . '{postcode}';
			}
			...
			$data['addresses'][] = array(
				...
				'contact'     => $this->url->link('account/address/contact', 'address_id=' . $result['address_id'], 'SSL')
			);
		}
	...
	public function contact() {
		...
		if (isset($this->request->get['address_id'])&& $this->valicontact()) {
			$this->model_account_address->EditContactY($this->request->get['address_id']);
			...
			$this->session->data['success'] = $this->language->get('error_contactn');
			...
			$this->model_account_activity->addActivity('address_contact', $activity_data);
	...
	protected function valicontact() {

		$results = $this->model_account_address->getAddresses();
            foreach ($results as $result){
            }
        if ($result['state_contact'] == 'y') {
        	# code...
        	$this->error['warning'] = $this->language->get('error_contacty');
        }
